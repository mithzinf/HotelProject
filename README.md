...
---
## 프로젝트를 진행하며 겪었던 문제점과 해결방안

첫번째.  
problem : 실시간 리뷰 데이터 조회 시 새로고침 필수로 인한 불편함  
solution : 새로고침 없이 페이지 리로드 구현  
result : Ajax를 통해 새로고침 없이 데이터 불러오기 성공  

두번째.  
problem : 같은 객실 동시 예약 기능 부재로 인한 에러 발생  
solution : 동시성 제어 관련 기능 구현 필요  
result : 많은 고민 끝에, 낙관적 락킹 기법 활용하여 예약 교착 상태 에러 방지  
...
3. jQuery, ajax를 활용해보고 싶었는데, 이번 기회를 통해 목표를 실현할 수 있어 보람찼다. ajax 방식을 사용하여 코딩을 만들어보니 왜 ajax를 사용하는지 와닿았고, json 형식의 데이터를 사용하는지 확실히 체감할 수 있었다.

## 개선사항
1. 현재 Mybatis 프레임워크로 개발된 프로젝트를 Spring Data JPA로 마이그레이션 하고싶다.
2. ajax 말고 Web Socket을 활용하여, 새로고침 없이 리로드 하여 데이터를 불러오고싶다.
3. HotelList 페이지 - redis & cache을 통해 리팩토링하여 가볍고, 로딩을 더 빠르게 구현하고싶다.
4. 현재, 낙관적 락킹을 활용하여 동시성 이슈를 해결했지만 다른 락킹 방법도 습득하여 적용해보고싶다.
5. 마이그레이션까지 마친 뒤 배포를 해보고싶다.
